...
async function createToken(): Promise<void> {
  ...
  await putParameter(paramName, token, `API token for auth-demo-c ${stage} environment`);
...
async function rotateToken(): Promise<void> {
  ...
  await putParameter(paramName, newToken, `API token for auth-demo-c ${stage} environment (rotated)`);
